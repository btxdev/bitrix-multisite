...
if (isset($arParams["ROOT_MENU_TYPE"]) && $arParams["ROOT_MENU_TYPE"] <> '')
	...
	$arParams["ROOT_MENU_TYPE"] = "footer";
...
if (isset($arParams["CHILD_MENU_TYPE"]) && $arParams["CHILD_MENU_TYPE"] <> '')
	...
	$arParams["CHILD_MENU_TYPE"] = "footer";
...
if (isset($arParams["MENU_TYPES"]) && is_array($arParams["MENU_TYPES"]) && !empty($arParams["MENU_TYPES"]))
{
	$arParams["MENU_TYPES"] = array_map(function($type) {
		return is_string($type) && $type !== '' ? htmlspecialcharsbx(trim($type)) : 'footer';
	}, $arParams["MENU_TYPES"]);

	$arParams["ROOT_MENU_TYPE"] = isset($arParams["MENU_TYPES"][0]) ? $arParams["MENU_TYPES"][0] : 'footer';
	$arParams["CHILD_MENU_TYPE"] = isset($arParams["MENU_TYPES"][1]) ? $arParams["MENU_TYPES"][1] : 'footer';
}
else
{
	$arParams["MENU_TYPES"] = [
...
if($USER->IsAuthorized())
{
	...
	if($bMenuAdd || $bMenuEdit || $bMenuDelete)
	{
		...
		$bDefaultItem = ($curDir == "/" && $menuType == "header" || $curDir <> "/" && $menuType == "footer");
		$buttonID = "menus";
	}
